...
- 게임화면에는 플레이어1과 플레이어2의 화면이 중간 세로줄을 기준으로 좌측은 플레이어1, 우측은 플레이어2의 화면이 나온다.
### 요구 사항에 따른 작업 내용
1. 미리 지정한 사이즈의 미로 제작후 크기 변경 및 플레이어 크기 확정
2. 플레이어에 카메라 하위 배치
3. 각각의 플레이어가 순서에 알맞게 풀어 나갈 수 있도록 함정 배치
4. 색상은 최대한 비슷한 색으로 배치
5. 한 화면에 카메라를 2대 위치시키기 위해 카메라 위치 수정
...
- 게임종료 조건은 따로 없으며 ESC를 누를 시 나가기 버튼이 화면 중간에 나오게 된다.
### 요구 사항에 따른 작업 내용
1. 발판(큐브)에 trigger오브젝트를 놓아 함정과 상호작용 하도록 함
2. 플레이어 중심점으로 발동이 힘들어 플레이어 오브젝트와의 충돌을 기준으로 함
3. 시작화면을 중심으로 설정화면, 설명화면을 추가로 띄을 수 있게 제작
4. 게임화면은 시작화면에서 다른 Scenes으로 이동한 후 시작
5. esc버튼은 설명화면에서 설명, 게임화면에는 따로 없다.
...
- 플레이어1이 서버이기 때문에 플레이어2만으로는 게임을 할 수 없다.
### 요구 사항에 따른 작업 내용
1. 시작 버튼 클릭시 Scenes변경
2. 설명화면과 설정버튼 안의 택스트작업 및 오브젝트 작업
3. 서버/네트워크 작업을 통해 서로의 움직임이 보이는지 확인
4. ESC버튼을 누른 후 다시 게임에 들어왔을 때 게임이 정상적으로 진행되는지 확인
...
- 플레이어1이 서버이기 때문에 플레이어2만으로는 게임을 할 수 없다.**
### 요구 사항에 따른 작업 내용
1. 기본함정에 Trigger오브젝트 추가, 플레이어의 위치 이동기능 추가
2. 클리어 씬 제작, 클리어 축하 메시지와 제작에 도움을 준 에셋 링크 추가
3. 서버/네트워크 안정화
